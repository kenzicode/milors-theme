...
Author URI: http://www.pankajmondal.com
        ...
        public function widget($args, $instance) {
            ...
            <div class="pwn-profile-widget-ninja-wrapper pwn-profile-widget-ninja-layout-<?php echo (isset( $instance['pwn_alignment'])) ? $instance['pwn_alignment'] : 'center'; ?> pwn-layout-style-<?php echo (isset( $instance['pwn_layout_style'])) ? $instance['pwn_layout_style'] : 'style1'; ?> pwn-profile-widget-ninja-avatar-<?php echo (isset($instance['pwn_profile_image_shape'])) ? $instance['pwn_profile_image_shape'] : ''; ?>"<?php if (!empty($widget_custom_background_color)) { echo ' style="' . $widget_custom_background_color . '"'; } ?>>
                ...
                if ($social_media_links_exist === 'y') {
                    ...
                    // Email ID
                    if (isset($instance['pwn_email_id']) && !empty($instance['pwn_email_id'])) {
                        echo '<a href="mailto:'. $instance['pwn_email_id'] .'" target="_blank"><span class="pwn-social-icon email-'.$iconset.'" title="Email"></span></a>';
                    }
                    // Facebook URL
                    if (isset($instance['pwn_facebook_url']) && !empty($instance['pwn_facebook_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_facebook_url']) .'" target="_blank"><span class="pwn-social-icon facebook-'.$iconset.'" title="Facebook"></span></a>';
                    }
                    // Twitter URL
                    if (isset($instance['pwn_twitter_url']) && !empty($instance['pwn_twitter_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_twitter_url']) .'" target="_blank"><span class="pwn-social-icon twitter-'.$iconset.'" title="Twitter"></span></a>';
                    }
                    // LinkedIn URL
                    if (isset($instance['pwn_linkedin_url']) && !empty($instance['pwn_linkedin_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_linkedin_url']) .'" target="_blank"><span class="pwn-social-icon linkedin-'.$iconset.'" title="LinkedIn"></span></a>';
                    }
                    // Google+ URL
                    if (isset($instance['pwn_googleplus_url']) && !empty($instance['pwn_googleplus_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_googleplus_url']) .'" target="_blank"><span class="pwn-social-icon googleplus-'.$iconset.'" title="Google+"></span></a>';
                    }
                    // YouTube URL
                    if (isset($instance['pwn_youtube_url']) && !empty($instance['pwn_youtube_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_youtube_url']) .'" target="_blank"><span class="pwn-social-icon youtube-'.$iconset.'" title="YouTube"></span></a>';
                    }
                    // Vimeo URL
                    if (isset($instance['pwn_vimeo_url']) && !empty($instance['pwn_vimeo_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_vimeo_url']) .'" target="_blank"><span class="pwn-social-icon vimeo-'.$iconset.'" title="Vimeo"></span></a>';
                    }
                    // GitHub URL
                    if (isset($instance['pwn_github_url']) && !empty($instance['pwn_github_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_github_url']) .'" target="_blank"><span class="pwn-social-icon github-'.$iconset.'" title="GitHub"></span></a>';
                    }
                    // Blogger URL
                    if (isset($instance['pwn_blogger_url']) && !empty($instance['pwn_blogger_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_blogger_url']) .'" target="_blank"><span class="pwn-social-icon blogger-'.$iconset.'" title="Blogger"></span></a>';
                    }
                    // Skype URL
                    if (isset($instance['pwn_skype_url']) && !empty($instance['pwn_skype_url'])) {
                        echo '<a href="skype:'. $instance['pwn_skype_url'] .'"><span class="pwn-social-icon skype-'.$iconset.'" title="Skype"></span></a>';
                    }
                    // Instagram URL
                    if (isset($instance['pwn_instagram_url']) && !empty($instance['pwn_instagram_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_instagram_url']) .'" target="_blank"><span class="pwn-social-icon instagram-'.$iconset.'" title="Instagram"></span></a>';
                    }
                    // Pinterest URL
                    if (isset($instance['pwn_pinterest_url']) && !empty($instance['pwn_pinterest_url'])) {
                        echo '<a href="'. esc_url($instance['pwn_pinterest_url']) .'" target="_blank"><span class="pwn-social-icon pinterest-'.$iconset.'" title="Pinterest"></span></a>';
                    }
                    echo '</div>';
                }
...
function enqueue_style_profile_widget_ninja() {
    wp_enqueue_style('profile-widget-ninja-frontend', plugins_url('css/profile_widget_ninja.css', __FILE__), array(), 4.1 );
}
